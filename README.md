...
## 作者声明
- 本仓库为个人学习原型。
- 已实现Agent编排、工具治理、RAG、长任务恢复、评测框架等底层逻辑。
- 暂未接入完整可用的外部软件Skill集合，无法直接调用软件完成真实业务任务。
- 目前依然在不断升级中，敬请期待

- 仅用于技术学习、研究与演示，请勿直接投入生产使用。
- 欢迎技术讨论，请勿使用商用产品标准评判该项目。
...
- [docs/TASK_UNDERSTANDING_ROUTING_PREFLIGHT.md](docs/TASK_UNDERSTANDING_ROUTING_PREFLIGHT.md) — 任务画像（TaskProfile）、路由硬约束与能力预检
