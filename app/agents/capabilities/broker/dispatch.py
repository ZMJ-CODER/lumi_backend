...
1. 能力名 → MCP 原子工具名（:data:`app.agents.capabilities.registry.builtin.TOOL_CAPABILITY_MAP` 的反向映射）；
...
# 候选租约选择（纯决策）已迁到 backend-neutral 的 ``lumi_capability.selection``。
from lumi_capability.selection import select_lease as _pkg_select_lease
...
CAPABILITY_TOOL_MAP: dict[str, str] = {
    ...
    # （app/workspace/write/operations.py）在客户端原子工具之上编排；这里登记
...
def mcp_tool_for_capability(capability: str, *, fallback: str = "") -> str:
    ...
    **真相源**：``TOOL_REGISTRY_DERIVED`` 打开时先问统一注册表；关闭时（默认）逐字走
    ...
    try:
        from app.agents.capabilities.catalog.tool_registry import registry_derived_enabled, resolve_tool
...
def _registry_entries() -> list[Any]:
    from app.agents.capabilities.catalog.tool_registry import build_registry_entries
...
def select_lease(
    ...
    *,
    ...
    纯核在 :func:`lumi_capability.selection.select_lease`；本函数保留原签名
    （吃 ``AgentExecutionContext``）并把回退事件交回 loguru（内核不依赖日志库）。
    """
    return _pkg_select_lease(
        leases,
