...
def _is_abstract(name: str) -> bool:
    return str(name or "").strip().upper() in ABSTRACT_CAPABILITY_MAP
...
def resolve_declared(entries: list[str]) -> tuple[list[str], list[NodeCapabilityIssue]]:
    ...
    for entry in entries:
        ...
        if not _is_abstract(text):
            if text not in concrete:
                concrete.append(text)
            continue
        mapped = ABSTRACT_CAPABILITY_MAP.get(text.upper(), ())
        if not mapped:
            issues.append(
                NodeCapabilityIssue(
                    capability=text,
...
def evaluate_node_capabilities(
    ...
    *,
    ...
    if not entries:
        return gate
    concrete, abstract_issues = resolve_declared(entries)
...
def node_capability_failure(
    ...
    *,
    ...
    gate = evaluate_node_capabilities(
        node, catalog=catalog, resolver=resolver, deployment=deployment, binding=binding
    )
    if gate.ok:
        return None
