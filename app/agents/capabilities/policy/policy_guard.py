...
from app.agents.capabilities.policy.policy_packs import PolicyPack, policy_packs

# 参数指纹的纯核已迁到 backend-neutral 的 ``lumi_capability.fingerprint``（P3 第二批）；
# 本模块保留同名公开名（既有调用点与测试都从这里取），实现由包提供。
...
class PluginPolicyGuard:
    ...
    def pack_for(self, policy_id: str = "") -> PolicyPack:
        ...
        if found is not None:
            return found
        from app.agents.capabilities.policy.policy_packs import DEFAULT_POLICY_ID
