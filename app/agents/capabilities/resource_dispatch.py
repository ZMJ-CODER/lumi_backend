...
from app.agents.capabilities.resource_surface import display_name_for
...
def adapter_for(capability: str, resource_type: str) -> ProviderAdapter | None:
    """首选 Adapter：第一个**已注册**的候选（只有一个声明、没有实现时返回 None）。"""
...
def resolve_dispatch(
    ...
    *,
    ...
    mcp_target = tool
    try:
        from app.agents.capabilities.tool_registry import resolve_tool
...
def dispatch_labels(tool_name: str) -> dict[str, str]:
    ...
    输出五个字段（值全部是闭集词汇，**不含参数、路径或任何用户输入**）：

    ``capability``     统一能力（``resource.write``）
    ``resource_type``  资源类型（``workspace``）
    ``provider_id``    物理 Provider（``lumi.local.workspace``）
    ``provider_name``  逻辑 Provider（``workspace_provider``）
    ``display_name``   模型可见名（收敛关闭时等于工具名，打开时是 ``Write`` 这类对外名）

    认不出工具就返回空字典——"不认识"不等于"可以编一个能力"。工具名本身也过形状闸门：
    带空格/斜杠/引号的输入直接拒绝，避免有人把参数拼进工具名里带出来。
    """
    from lumi_contracts.events.process import label_value
