...
无论开关状态，:func:`shadow_compare` 都能把"静态 vs 派生"的差异打点出来——这是切换
真相源之前必须看的证据（沿用本项目既有的影子期风格）。
"""
...
def _static_mcp_target(capability: str) -> str:
    ...
    if not base:
        return ""
    try:
        from app.agents.capabilities.dispatch import CAPABILITY_TOOL_MAP
...
def mcp_target_for(capability: str, *, fallback: str = "") -> str:
    ...
    if not registry_derived_enabled():
        from app.agents.capabilities.dispatch import mcp_tool_for_capability
...
def _stricter(first: str, second: str) -> str:
    ...
    order = {TIER_AUTO: 0, TIER_ROUTINE: 1, TIER_CRITICAL: 2}
    candidates = [item for item in (str(first or ""), str(second or "")) if item in order]
    if not candidates:
        return ""
    return max(candidates, key=lambda item: order[item])


def _normalize_tier(value: Any) -> str:
    text = str(value or "").strip().casefold()
    return text if text in {TIER_AUTO, TIER_ROUTINE, TIER_CRITICAL} else ""
...
def side_effect_tier(effects: Any) -> str:
    ...
    if not names:
        return ""
    tiers = {SIDE_EFFECT_TIER.get(name, TIER_ROUTINE) for name in names}
...
def capability_declared_tier(capability: str) -> str:
    ...
    **什么都不声明的能力返回 ``""``**（交给调用方的保守默认），而不是猜一个 ``auto``。
    ...
    if descriptor is None:
        return ""
    tier = side_effect_tier(getattr(descriptor, "side_effects", None))
    if not tier:
        return ""
    return _floor_for_local_confirmation(
...
def manifest_tier_of(manifest: Any) -> str:
    ...
    """
    tier = side_effect_tier(getattr(manifest, "side_effects", None))
    if not tier:
        return ""
    return _floor_for_local_confirmation(tier, manifest_requires_local_confirmation(manifest))
...
def shadow_compare() -> dict[str, list[list[str]]]:
    ...
    # 维度 3：动作意图 → 工具窗口
    try:
        from app.agents.orchestration.capability_preflight import ACTION_TOOL_WINDOW
