...
#: 工具名必须与 ``app/agents/capabilities/catalog.py`` 的注册名一致（由
#: ``tests/test_task_understanding_preflight.py`` 断言），否则预检会"通过"却注入不存在
#: 的工具，模型又只能自己编理由。
...
def tool_window_for_actions(
    ...
    **工具窗口的真相源**：``TOOL_REGISTRY_DERIVED`` 打开时由统一工具注册表派生
    ...
    for intent in action_intents or ():
        ...
        try:
            from app.agents.capabilities.tool_registry import action_window
