"""Immutable request context shared by planning entry points.

The public Planner API still accepts the historical positional arguments for
plugins and tests.  Internally, orchestration code can pass this object so a
new planning concern does not require another long parameter chain.
"""
...
class PlanRequestContext:
    """All user-controlled inputs needed to produce a task tree."""
    ...
    # Frozen request-scoped model selection.  Contains the key only in memory
    # or the short-lived job secret bridge; never expose it in job metadata.
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(
            self,
            "project_ids",
            tuple(str(value) for value in (self.project_ids or ()) if str(value).strip()),
        )
        object.__setattr__(
            self,
            "office_docs",
            tuple(dict(item) for item in (self.office_docs or ()) if isinstance(item, Mapping)),
        )
        object.__setattr__(self, "prior_summaries", str(self.prior_summaries or ""))
        object.__setattr__(
            self,
            "recent_messages",
            tuple(str(value) for value in (self.recent_messages or ()) if str(value).strip()),
        )
        object.__setattr__(
            self,
            "recent_artifacts",
            tuple(dict(item) for item in (self.recent_artifacts or ()) if isinstance(item, Mapping)),
        )
        ...
        object.__setattr__(
            self,
            "permissions",
            tuple(str(value) for value in (self.permissions or ()) if str(value).strip()),
        )
    ...
    def from_legacy_args(
        ...
        clarification_answer: str | None = None,
        office_docs: list[dict] | tuple[dict, ...] | None = None,
        prior_summaries: str = "",
        ...
        return cls(
            ...
            llm_config=dict(llm_config) if llm_config else None,
            clarification_answer=clarification_answer,
            office_docs=tuple(office_docs or ()),
            prior_summaries=prior_summaries,
            recent_messages=tuple(recent_messages or ()),
            recent_artifacts=tuple(recent_artifacts or ()),
    ...
    def from_mapping(cls, values: Mapping[str, Any]) -> "PlanRequestContext":
        """Rebuild a context from the in-memory legacy job context mapping."""
        return cls.from_legacy_args(
            user_id=values.get("user_id", ""),
            request=values.get("request", ""),
            scene=values.get("scene", "office"),
            project_id=values.get("project_id"),
            project_ids=values.get("project_ids"),
            llm_api_key=values.get("llm_api_key"),
            llm_config=values.get("llm_config"),
            clarification_answer=values.get("clarification_answer"),
            office_docs=values.get("office_docs"),
            prior_summaries=values.get("prior_summaries", ""),
            recent_messages=values.get("recent_messages"),
            recent_artifacts=values.get("recent_artifacts"),
            previous_plan=values.get("previous_plan"),
    ...
    def as_legacy_args(self) -> tuple[Any, ...]:
        """Return the historical positional Planner argument tuple."""
        return (
            self.user_id,
            self.request,
            self.scene,
            ...
            [dict(item) for item in self.office_docs],
