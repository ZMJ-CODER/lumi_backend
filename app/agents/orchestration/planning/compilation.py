...
class PlanCompilationService:
    """无状态的计划编译门面；基础设施通过调用方注入。"""

    def __init__(self, *, workers: dict, plan_with_context: Callable[[PlanRequestContext], Awaitable[Any]], temporal_static_mode: bool = False) -> None:
    ...
    async def compile_with_feedback(self, tree, *, routing: dict, context: PlanRequestContext, user_role: str):
        ...
        async def compile_current(current_tree):
            ...
            if self._temporal_static_mode:
                ...
                decision = evaluate_static_temporal_nodes(
                    ...
                    max_nodes=int(getattr(settings, "TEMPORAL_STATIC_MAX_NODES", 12)),
                    long_dag_enabled=bool(getattr(settings, "TEMPORAL_STATIC_LONG_DAG_ENABLED", False)),
                    long_dag_max_nodes=int(getattr(settings, "TEMPORAL_STATIC_LONG_DAG_MAX_NODES", 12)),
                )
                if decision.eligible:
                    max_nodes = int(getattr(settings, "TEMPORAL_STATIC_LONG_DAG_MAX_NODES", 12)) if len(current_tree.nodes or []) > int(getattr(settings, "TEMPORAL_STATIC_MAX_NODES", 12)) else int(getattr(settings, "TEMPORAL_STATIC_MAX_NODES", 12))
            result = await compile_plan(current_tree.nodes, scene=context.scene, user_role=user_role, user_id=context.user_id, workers=self._workers, max_nodes=max_nodes)
        ...
        if compiled.decision == CompileDecision.REPLAN_REQUIRED:
            ...
            logger.warning("计划编译器拒绝规划，触发一次带反馈重规划: user={} reason={}", context.user_id[:8], feedback[:300])
            retry_summaries = ((context.prior_summaries + "\n") if context.prior_summaries else "") + "上一版计划未通过执行前编译检查。请修正以下约束后重新输出完整 JSON，不要删除用户要求的动作：" + feedback[:1200]
            try:
                revised = await self._plan_with_context(context.with_prior_summaries(retry_summaries))
...
__all__ = ["PlanCompilationService"]
