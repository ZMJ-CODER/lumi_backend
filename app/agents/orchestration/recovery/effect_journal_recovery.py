...
* **本模块**是应用侧的唯一收口：从既有 Effect Journal（
  :mod:`app.agents.orchestration.runtime.effects`，不新建第二套日志）读取状态，再叠加
...
:class:`app.agents.orchestration.recovery.failed_job_recovery_service.FailedJobRecoveryService`）
...
async def _default_reader(key: str) -> dict[str, Any] | None:
    from app.agents.orchestration.runtime.effects import get_effect
