...
from app.office.api import resolve_generic_output

# ── [纯决策] 任务引用协议：本模块**只依赖这些事实**，不依赖编排模型 ─────────
# 方案（P4 域 3）要求切断 office_task_memory → app.agents.orchestration.models 的反向依赖。
# 这里刻意**不新建复杂 Port**：只声明"我需要任务的哪几个字段"，任何一个带这些字段的
# 对象都能传进来（编排的 ``Job`` 天然满足）。状态比较用**字符串值**而不是枚举成员，
# 于是本域不再 import 编排内核，也不再因为编排改枚举而跟着改。
_TERMINAL_STATUSES: frozenset[str] = frozenset({"completed", "failed", "cancelled", "interrupted"})
_COMPLETED_STATUS = "completed"


def _status_value(status: Any) -> str:
    """``JobStatus.COMPLETED`` / ``"completed"`` 两种形状都取到同一个字符串。"""
    return str(getattr(status, "value", status) or "")


@runtime_checkable
class TaskSnapshot(Protocol):
    """本模块消费的**任务事实**（编排的 ``Job`` 天然满足这个形状）。"""
...
def _artifact_refs(job: TaskSnapshot) -> list[dict]:
    """Extract only client-safe artifact identity from successful node results."""
    found: list[dict] = []
    for node in job.nodes:
        if _status_value(node.status) != _COMPLETED_STATUS or not isinstance(node.result, dict):
            continue
...
def _result_refs(job: TaskSnapshot) -> list[dict]:
    refs: list[dict] = []
    for node in job.nodes:
        if _status_value(node.status) != _COMPLETED_STATUS:
            continue
...
async def upsert_office_task_index(session: AsyncSession, job: TaskSnapshot) -> None:
    """Persist one terminal task. Repeated status polling is intentionally idempotent."""
    if job.scene != "office" or _status_value(job.status) not in _TERMINAL:
        return
    ...
    *,
    ...
    rows = (
        await session.execute(
            ...
            .where(OfficeTaskIndex.user_id == user_id, OfficeTaskIndex.status == _COMPLETED_STATUS)
