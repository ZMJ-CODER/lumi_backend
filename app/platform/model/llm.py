...
from app.platform.runtime.resilience import get_breaker, is_transient_dependency_error
...
#: 缺少模型凭据时的稳定错误码（前端据此弹"去填 API Key"，而不是显示"服务器内部错误"）。
ERROR_CODE_MODEL_API_KEY_MISSING = "MODEL_API_KEY_MISSING"


class ModelCredentialsMissingError(AppException):
    """没有可用的模型 API Key：**结构化**告诉调用方去补什么。

    以前这里会让 ``ChatOpenAI(api_key="")`` 直接抛 ``openai.OpenAIError: Missing
    credentials``，一路冒到全局兜底 → **500 服务器内部错误**。用户看到的只是"发消息失败"，
    既不知道要填 Key，也不知道是自带密钥（BYOK）没带上。两种情形分开说：

    * BYOK：用户选了自带密钥，但请求没带 ``x-llm-api-key``（前端没填/本地存储丢了）；
    * 服务端密钥：``.env`` 里的 ``DEEPSEEK_API_KEY`` / ``QWEN_API_KEY`` 为空。

    本地/私有端点（localhost、内网地址）**允许空密钥**——自建推理服务（Ollama/vLLM）
    常常不校验密钥，不能因为这条守卫把它们一刀切掉。
    """

    def __init__(self, message: str, *, byok: bool = False, base_url: str = "") -> None:
        super().__init__(
            status_code=400,
            message=message,
            code=400,
            data={"error_code": ERROR_CODE_MODEL_API_KEY_MISSING, "byok": byok, "base_url": base_url},
            error_code=ERROR_CODE_MODEL_API_KEY_MISSING,
        )


def _is_local_endpoint(base_url: str) -> bool:
    """本地/内网端点判定（这些端点允许不带密钥）。"""
    try:
        from urllib.parse import urlparse

        host = (urlparse(str(base_url or "")).hostname or "").casefold()
    except Exception:  # noqa: BLE001 - 解析失败按"远端"处理（保守）
        return False
    if not host:
        return False
...
class LLMClient:
    ...
    async def _model(
        ...
        **角色档位**（``role=``，见 ``app.platform.model.model_roles``）→ 既有 scene 链。
        ...
        if role_name and llm_config is None and not (model or base_url):
            ...
            if require_tools and not role_info.supports_tools:
                ...
                from app.platform.model.model_roles import PROFILE_MAIN
        ...
        # 创建客户端前就判"有没有密钥"：让空密钥变成**可行动的 400**，而不是
        # ChatOpenAI 抛 OpenAIError 后变成 500。本地/内网端点允许空密钥。
        if not str(selected_api_key).strip() and not _is_local_endpoint(selected_base_url):
            # 只记录"缺少凭据"这一事实与来源（byok/base_url/用户前缀），永不打印密钥本身。
            logger.warning(
                "[model] 缺少模型凭据：byok={} base_url={} user={}（若为 BYOK，请检查请求头 x-llm-api-key）",
                bool(cfg.get("byok")),
                selected_base_url or "<empty>",
                str(user_id or "")[:8] or "-",
            )
            raise ModelCredentialsMissingError(
                (
                    "当前使用「自带密钥」，请在设置里填写模型 API Key 后重试"
                    "（发消息时通过请求头 x-llm-api-key 携带）"
                    if cfg.get("byok")
                    else "服务端没有配置模型 API Key：请在 .env 配置 DEEPSEEK_API_KEY/QWEN_API_KEY，"
                    "或在设置里改用自带密钥"
                ),
                byok=bool(cfg.get("byok")),
        ...
        from app.platform.runtime.deadline import ensure_budget, request_budget_seconds
