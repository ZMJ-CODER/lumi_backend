...
async def resolve_effective_llm_config(
    *,
    ...
    if bool(cfg.get("byok")) and not request_api_key and not str(cfg.get("api_key") or "").strip():
        from app.platform.model.model_catalog import PROVIDER_BASE_URLS, normalize_provider_base_url
...
async def _resolve_user_cfg(user_cfg: dict, provider: str | None) -> dict | None:
    ...
    """
    from app.platform.model.model_catalog import PROVIDER_BASE_URLS, normalize_model_id

    prov = user_cfg.get("provider") or provider
    base_url = ""
    if user_cfg.get("byok") and user_cfg.get("base_url"):
        from app.platform.model.model_catalog import normalize_byok_base_url
