...
* 候选**只**来自两处——``app/platform/model/model_catalog.py``（模型注册表：上下文/多模态）
  与 ``.env``（含管理员动态覆盖）配置出来的 ``main`` / ``cheap`` / ``reasoning`` / ``vision``
  档位；解析走 ``app/platform/model/model_roles.py``，任务内一律以 ``app/platform/model/model_plan.py`` 的
  **冻结计划**为准（不新造第二条选择路径）；
...
from app.platform.model.model_plan import ModelPlan, profile_user_label
...
class ModelCapabilityRouter:
    ...
    def _catalog_items(self) -> list[dict[str, Any]]:
        ...
        from app.platform.model.model_catalog import get_model_catalog
