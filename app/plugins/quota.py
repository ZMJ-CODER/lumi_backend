...
``enforce_quota()``、不杀进程、不落产物（见 ``tests/capabilities/test_plugin_quota_enforcement.py``）。
"""
...
# ── 执行证据（只由真实执行边界写；见 app.plugins.boundary） ──
...
def _record_evidence(
    ...
    *,
    ...
    try:
        from app.plugins.boundary import record_execution
...
def _record_refusal(spec: PluginQuotaSpec, *, detail: str = "") -> None:
    try:
        from app.plugins.boundary import record_refused
