...
改造前逐字节一致（见 ``tests/test_plugin_lifecycle_wiring.py`` 的等价性用例）。
"""
...
class PluginManager:
    ...
    def worker_for(
        ...
        *,
        ...
        """
        from app.services.plugins.quota import PluginWorker, quota_spec_for
    ...
    def quota_status(self, plugin_id: str) -> dict[str, Any]:
        ...
        唯一口径在 ``app.services.plugins.boundary``：只有"开关打开 + 硬约束 +
        真走过可强杀进程"才算 ``enforced``，协作式**不算**。
        """
        from app.services.plugins.boundary import plugin_quota_status

        return plugin_quota_status(self, plugin_id, settings=self._settings)

    def quota_statuses(self) -> list[dict[str, Any]]:
        """全部已安装插件的配额状态（列表接口用）。"""
        from app.services.plugins.boundary import plugin_quota_statuses
    ...
    async def _mark_inflight_uncertain(self, plugin_id: str) -> list[str]:
        ...
        if not rows:
            return marked
        from app.agents.orchestration import effects
...
def active_plugin_manager() -> PluginManager | None:
    ...
    if not feature_enabled(FLAG):
        return None
    from app.services.plugins.registry import PluginRegistry as _Registry
    from app.services.plugins.state import PluginStateStore as _Store
