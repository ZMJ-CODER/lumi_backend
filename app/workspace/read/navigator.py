...
# ── [读] action=read：正文读取（行窗口 / 全文 / 分段投影）
class NavigatorReadHandler:
    ...
    async def _read_line_window(
        ...
        """
        service = self.service
        from app.knowledge.api import slice_lines
...
class NavigatorScanHandler:
    ...
    解析完全在服务端做（``app/knowledge/code/code_structure.py``，纯函数）：即使客户端只提供
    ...
    async def run(self, args: dict) -> dict:
        ...
        full_text = "\n".join(str(item.get("text") or "") for item in sections)
        reader_meta = payload.get("meta") if isinstance(payload.get("meta"), dict) else {}
        from app.knowledge.api import scan_text, slice_lines
        ...
        found = None
        if find_name:
            from app.knowledge.api import find_symbol
...
def model_text(payload: dict, *, limit: int = MODEL_TEXT_MAX_CHARS) -> str:
    ...
    if action == ACTION_SCAN:
        from app.knowledge.api import render_skeleton_lines
