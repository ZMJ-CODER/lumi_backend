...
TCA 的规则命中仍是默认生产路径。规则无法识别或置信度偏低时，规划器可调用一次严格 JSON 的轻量长尾分类器；请求级 BYOK `llm_api_key` 优先，
没有则使用统一的办公/全局预设密钥。该调用只返回动作/对象候选，不生成节点，也不能绕过后续安全检查。没有有效密钥、调用失败或 JSON 不合规时，
系统保守澄清，不把未知请求伪装成 RAG。
...
当普通办公 DAG 的节点数达到 `AGENT_LOGICAL_PLAN_MIN_NODES` 时，完整计划会外置为逻辑计划，`Job.nodes` 只保留当前依赖已满足的执行前沿。
每轮结束后，编排器提交节点状态和不含正文的 `result_ref`，检查预算后再物化下一前沿。下游需要上游内容时，只能在执行时按用户归属和哈希校验读取该引用，
不能从 Job 快照获得完整前缀。
...
工具目录不会全量注入模型。注册表先完成场景、角色、写开关、运行时可用性和资源范围过滤；随后按
`domain`、`intent_tags`、`use_when` 和语义分数取请求级 Top-K。聊天默认最多 5 个候选，若没有
任何 Skill 对当前请求具有正向证据则不给模型工具，直接回答。每个候选还带 `do_not_use_when` 与
相邻 Skill 的显式边界，例如网页检索排除用户附件、知识库和当前系统时间。模型因此先排除不该调用的
能力，再在最小合法候选集中选择；向量分数只用于合法池排序，绝不能扩大权限或绕过审批。
...
- `use_when`、`do_not_use_when` 和带正反例的 `selection_examples`；每次新增或修改 Skill 都要补
  零执行选择评测，包含“应调用”和最易混淆的 `must_not_call`。

成功结果除正文和 citation 外还回填 `decision_signals`（结果数、置信提示、是否截断、是否还有更多
结果及细化建议）。失败结果回填可操作的参数/权限/审批提示，而不是裸错误码；模型据此收窄查询、读取
候选或向用户澄清，不能靠尝试其他写工具绕过门禁。

工具候选 Top-K 本身是独立路由层：候选召回负责“正确工具是否进入可见池”，模型选择负责“已见工具是否被正确调用”。两层分别以 `candidate_recall@K` 和 `selection_accuracy_given_candidates` 评测，不能用最终调用记录混为一谈。候选 trace 只持久化工具名、版本、分数、是否 bootstrap、轮次和最终调用/未调用候选；不包含用户原文、提示词、参数或工具输出。聊天使用一次请求池；办公 ReAct 在每轮结合最新观察和失败工具刷新候选池，记录至节点 `tool_metadata.selection_traces`。

明确工具意图却没有候选或仅低分候选时会产生 `TOOL_CANDIDATE_LOW_CONFIDENCE` 监控事件，并要求回答不能伪称已核验；这只是召回漏失告警，不会退化为关键词强制联网。新 Skill 可在限定意图和到期日期内使用 bootstrap 进入候选池，期限结束后自动回归普通排序。注册表 lint 会校验相邻工具引用、bootstrap 日期/意图，以及高重叠标签的显式关系；候选提示从注册契约编译，避免提示词与实现分叉。

`confidence_hint` 不再接受自由文本：仅当结果能提供 `{level, basis[]}`（例如 citation 数、授权文档读取或供应商结果数）才回填给模型；无依据时省略该字段。
...
- 候选召回与选择：候选池、分数、bootstrap、模型最终调用和低置信召回漏失告警；
