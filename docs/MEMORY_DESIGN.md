...
- 用户画像只对开发者（superadmin）和大模型开放，管理员不可见，用户不可自管理。

### 1.2 非目标（v1 不做）

- 不做用户可操作的记忆管理界面（用户只能通过注销账号删除全部数据）；
- 不做多用户记忆共享/社交记忆；
...
| D4 | 用户不可管理自己的记忆；superadmin 可查看（调试）；admin 不可见 | 降低社工面，保留开发者排障能力 |
| D5 | 画像 + 事实库两级结构 | 画像常驻注入（便宜稳定），事实按需 RAG 召回（精确） |
| D6 | 服务端聊天记录按条数上限裁剪（保留 50、硬上限 70），物理删除，不做时间过期 | 匹配用户回看行为，控制存储并销毁聊天内 L2 PII |
...
- **检索验证（开发阶段）**：扩展 `scripts/benchmark_rag.py` 同套模式，新增记忆检索基准——构造"隐私事实 → 话题查询"用例（如"我最近血糖偏高" → 期望召回 `[L1][健康]`），指标 recall@1/3/5，验证占位符粒度不影响召回；
...
设计决策：服务端不做时间过期，采用与客户端**一致的条数上限**方案。理由：大多数用户不会回看超过一定轮次的对话，条数上限既天然销毁聊天中的 L2 PII，又控制存储增长，且多端同步语义一致。

规则（与客户端语义对齐）：

- `CONVERSATION_MESSAGE_KEEP = 50`：每会话保留最近 50 条消息；
- `CONVERSATION_MESSAGE_HARD_CAP = 70`：超过 70 条时触发裁剪，物理删除最旧的 20 条，回到 50 条；
- **物理删除**（不是软删除）：消息行 + 附件行（FK `ON DELETE CASCADE`）+ 附件磁盘文件一并清除；
- 游客会话不落库，无需裁剪。

触发方式（双层，防止漏触发）：

1. 消息写入提交后异步入队 `trim_conversation_messages(conversation_id)`（Celery，幂等，失败重试）；
2. 每日兜底任务 `cleanup_conversations()`（Celery beat）：扫描所有超过硬上限的会话并裁剪，兼容历史数据。
...
def trim_conversation_messages(self, conversation_id: str):
    """物理删除超出上限的最旧消息 + 附件文件."""
    # 1. SELECT id FROM messages WHERE conversation_id = :cid
    #    ORDER BY created_at DESC OFFSET :keep        # 保留最近 keep 条
    # 2. 收集将删消息的附件 file_url，逐一校验路径后删除磁盘文件
    # 3. DELETE FROM messages WHERE id IN (...)       # attachments 随 CASCADE 清理
```

边界处理：

- 裁剪只作用于 PostgreSQL 消息表，**不动 Redis 短期上下文**（已按轮次独立裁剪）；
- 删除附件文件前校验绝对路径位于 `uploads/chat/{user_id}/` 内，防止越权路径删除；
- 幂等重放依赖最近消息的 `client_message_id`，裁剪只删最旧消息，不影响重放；
- 会话软删除（`is_deleted`）仍保留最近 50 条，行为不变；
- 裁剪后多端同步一致：客户端本地同样只保留最近 50 条。

配置新增：

```python
CONVERSATION_MESSAGE_KEEP: int = 50
CONVERSATION_MESSAGE_HARD_CAP: int = 70
```

L2 隐私保证：精确 PII 不进入记忆库，且聊天记录最多在每会话保留 70 条内存在，超量即物理销毁（含附件文件）。

> 说明：条数上限约束的是"每会话"存储；总存储仍随会话数量增长。若日后需要，可再加"每用户活跃会话数上限"，v1 不做。
...
现有 `app/api/v1/memories.py` 桩改为 superadmin 调试接口（依赖已有的 `require_superadmin`）：

```python
GET    /admin/memories?user_id=xxx            # 列出某用户事实（L1 只回占位符）
PATCH  /admin/memories/{memory_id}            # 编辑/修正事实
DELETE /admin/memories/{memory_id}            # 删除事实
GET    /admin/memories/{user_id}/profile      # 查看某用户画像
```

- 用户侧记忆管理接口与前端入口**移除**（前端若已有记忆页，一并下线）；
- 用户数据删除走既有账号注销 → `delete_user_data`（已含 `DELETE FROM memories`，需补 `memory_profile`）。
...
### 阶段 5：聊天记录生命周期（可与阶段 2 并行）

- `CONVERSATION_MESSAGE_KEEP / HARD_CAP` 配置；
- `trim_conversation_messages` + `cleanup_conversations` Celery 任务；
- 消息写入后异步触发裁剪 + 附件文件物理清理（含路径校验）。

## 12. 风险与待确认

| 风险 | 说明 | 缓解 |
|------|------|------|
| 聊天附件文件残留 | 裁剪/删除消息时附件磁盘文件需同步清理，否则存储只减一半 | 删除前路径校验 + 物理删除任务统一处理（§8.3） |
