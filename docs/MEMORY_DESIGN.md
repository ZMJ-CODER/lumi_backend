...
| D6 | 原始会话消息不再按 UI 条数裁剪；聊天窗口与原文存储分离 | 原文是长对话按需回捞的证据，删除会话/账号才会级联删除 |
...
```text
L0 messages                 全量原文，唯一事实来源，不直接全量注入
...
### 8.3 聊天记录生命周期（原文保留与后续归档）

会话原文是长对话回捞的证据来源，不能再以 UI 显示窗口或固定条数为界物理裁剪。`messages` 是唯一原文来源；
L1/L2 只保存压缩摘要和消息引用，不能代替原文。

当前规则：

- `CONVERSATION_MESSAGE_KEEP = 0`、`CONVERSATION_MESSAGE_HARD_CAP = 0`：关闭按条数自动删除；
- Redis 仍仅保存有限的近期上下文，过期不影响 PostgreSQL 原文和 L1/L2 回捞；
- 用户删除会话或注销账号时，消息、会话段摘要、总摘要和关联附件按既有级联/清理流程删除；
- 游客会话不落库，因此没有跨会话记忆或历史回捞。

这项取舍以存储空间换取长对话的可追溯性。若存储压力出现，后续应把旧原文归档到低成本对象存储，并继续保留
消息 ID、内容哈希和可按需读取的引用；不得恢复固定条数裁剪，否则会破坏段摘要的原文证据链。

```python
# 0 表示禁用物理裁剪。删除会话/账号和未来归档策略才是生命周期边界。
