...
class CalendarManagerSkill(Tool):
    ...
    async def execute(self, params: dict, context: SkillContext | None = None) -> ToolOutput:
        ...
        if action in ("update", "delete"):
            ...
            if action == "delete":
                ...
                _save(context.user_id, events)
                return ToolOutput(success=True, output=f"已删除事件：{target.get('title')}")
            ...
            _save(context.user_id, events)
            return ToolOutput(success=True, output=f"已更新事件：{target.get('title')}\n\n{_fmt([target])}")
