...
class CreateOfficeDocumentSkill(Tool):
    ...
    async def execute(self, params: dict, context: SkillContext | None = None) -> ToolOutput:
        if not context or not context.user_id:
            return ToolOutput(success=False, error="需要登录后才能生成办公文档", error_code="INVALID_ARGS", retryable=False)
        ...
        try:
            ...
            return ToolOutput(success=False, error=f"文档生成失败：{str(exc)[:300]}", error_code="DOCUMENT_RENDER_FAILED", retryable=False)
        return ToolOutput(
            success=True,
