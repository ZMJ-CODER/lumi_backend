...
class InspectDocumentSetSkill(Tool):
    ...
    async def execute(self, params: dict, context: SkillContext | None = None) -> ToolOutput:
        ...
        if not doc_ids:
            return ToolOutput(success=False, error="没有可盘点的已授权办公文档", error_code="FORBIDDEN", retryable=False)
...
class OfficeDocReadSkill(Tool):
    ...
    async def execute(self, params: dict, context: SkillContext | None = None) -> ToolOutput:
        ...
        doc_id = str(params.get("doc_id") or "").strip()
        if not doc_id:
            return ToolOutput(success=False, error="缺少 doc_id", error_code="INVALID_ARGS", retryable=False)
        try:
            await office_docs.ensure_session(context.user_id, doc_id)
            info = await run_in_compute(office_docs.read_structure, context.user_id, doc_id)
        except LookupError as exc:
...
class OfficeDocEditSkill(Tool):
    ...
    write_op = True
    scenes = ["office"]
    # 编辑内容可由节点 instruction 直接传递；目标文档身份绝不能从“最近
    # 附件”或自然语言猜测，必须由规划结果显式绑定。
