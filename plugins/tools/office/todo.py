...
class TodoManagerSkill(Tool):
    ...
    async def execute(self, params: dict, context: SkillContext | None = None) -> ToolOutput:
        ...
        if action == "add":
            ...
            if not content:
                return ToolOutput(success=False, error="add 需要 content", error_code="INVALID_ARGS", retryable=False)
            items.append(
                {
                    "id": uuid.uuid4().hex[:8],
                    "content": content,
            ...
            return ToolOutput(success=True, output=f"已添加待办：{content}\n\n当前待办：\n{_fmt(items)}")
        if action == "list":
            return ToolOutput(success=True, output=_fmt(items))
        if action in ("complete", "delete"):
            item_id = str(params.get("item_id") or "").strip()
            target = next((x for x in items if x.get("id") == item_id), None)
            if not target:
                return ToolOutput(success=False, error="未找到该待办 id", error_code="INVALID_ARGS", retryable=False)
            if action == "complete":
                target["done"] = True
            ...
            _save(context.user_id, items)
            return ToolOutput(success=True, output=f"{msg}\n\n当前待办：\n{_fmt(items)}")
        return ToolOutput(success=False, error="action 仅支持 add/list/complete/delete", error_code="INVALID_ARGS", retryable=False)
