...
class PythonExecSkill(Tool):
    ...
    output_contract = {
        ...
        "budget": 1200,
        "truncate_strategy": "references_only",
        "on_overflow": "externalize",
    }
    parameters_schema = {
        "type": "object",
        "properties": {
    ...
    async def execute(self, params: dict, context: SkillContext | None = None) -> ToolOutput:
        ...
        if result.status == "success":
            ...
            return ToolOutput(
                ...
                data={"outputs": produced, "stdout_summary": (result.stdout or "(无输出)")[:500]},
                content_type="artifact",
                metadata={"outputs": produced, "artifact_refs": [item for item in produced if item.get("ref_id")]},
            )
