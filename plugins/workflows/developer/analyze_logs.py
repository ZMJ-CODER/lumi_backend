...
class AnalyzeLogsSkill(WorkflowSkill):
    name = "analyze_logs"
    description = (
    ...
    async def execute(self, params: dict, context: SkillContext | None = None) -> ToolOutput:
        ...
        if not (reply or "").strip():
            return ToolOutput(success=False, error="模型未返回分析结果", error_code="EXEC_ERROR", retryable=True)
        return ToolOutput(success=True, output=(reply or "").strip())
