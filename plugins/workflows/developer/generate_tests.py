...
class GenerateTestsSkill(WorkflowSkill):
    name = "generate_tests"
    description = (
    ...
    async def execute(self, params: dict, context: SkillContext | None = None) -> ToolOutput:
        ...
        if not content.strip():
            return ToolOutput(success=False, error="缺少被测代码 content", error_code="INVALID_ARGS", retryable=False)
        ...
        if not reply:
            return ToolOutput(success=False, error="模型未生成测试代码", error_code="EXEC_ERROR", retryable=True)
        if reply.startswith("```"):
            import re

            reply = re.sub(r"^```\w*\n?", "", reply)
            reply = re.sub(r"\n?```$", "", reply)
        return ToolOutput(success=True, output=reply, metadata={"path": path})
