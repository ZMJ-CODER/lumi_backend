...
注意：执行后需重嵌入已有文档与记忆（scripts/migrations/reembed_vectors.py）。

用法（读 DATABASE_URL）:
  docker compose exec api python scripts/migrations/migrate_bge_m3.py
"""
...
async def main() -> None:
    ...
    try:
        ...
        print("迁移完成。注意：旧向量已清空，需运行 scripts/migrations/reembed_vectors.py 重嵌入文档与记忆")
    finally:
        await conn.close()
