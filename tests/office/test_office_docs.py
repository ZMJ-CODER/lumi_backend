...
def test_analyze_doc_limits_direct_prompt_and_output(monkeypatch):
    ...
    monkeypatch.setattr(office_docs, "ensure_session", fake_ensure)
    monkeypatch.setattr("app.platform.runtime.executors.run_in_compute", fake_compute)
    ...
    # 这两个名字是**函数内 import**（analyze_doc 里现取），所以 patch 打在 api 模块上：
    # 函数执行时从 api 模块命名空间解析名字，替换 api 上的属性即可生效。
    monkeypatch.setattr("app.knowledge.api.search_user_knowledge", fake_search)
    monkeypatch.setattr("app.office.api.office_llm", fake_llm)
