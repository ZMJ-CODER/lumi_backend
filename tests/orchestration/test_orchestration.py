...
from app.agents.orchestration.admission import admission
...
from app.agents.orchestration.execution.workers import WORKERS, WorkerContext, list_workers
...
def _effect_journal_test_double():
    ...
    from app.agents.orchestration.runtime.effects import set_effect_journal_repository_for_tests
...
def test_admission_local_lease_can_only_renew_existing_job(monkeypatch):
    from app.agents.orchestration.admission.admission import JobAdmission
...
def test_atomic_step_switches_to_fallback_tool_on_retry(monkeypatch):
    ...
    monkeypatch.setattr("app.platform.model.llm.LLMClient", FakeLLM)
...
def test_atomic_document_step_executes_planned_tool_without_extra_llm(monkeypatch):
    ...
    monkeypatch.setattr("app.platform.model.llm.LLMClient", ForbiddenLLM)
    monkeypatch.setattr(atomic_mod, "get_tools_for_scene", fake_tools)
    monkeypatch.setattr(atomic_mod, "execute_tool_call", fake_execute)
    node = TaskNode(
        id="read",
...
def test_multilingual_output_filename_is_not_treated_as_input():
    from app.agents.orchestration.office.document_scope import extract_output_contract, select_named_office_documents
