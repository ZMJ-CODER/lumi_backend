...
def test_office_react_recomputes_tools_and_excludes_failed_method(monkeypatch):
    ...
    monkeypatch.setattr("app.agents.orchestration.react.runner.get_chat_model", lambda **kwargs: asyncio.sleep(0, result=model))
    monkeypatch.setattr("app.agents.orchestration.react.agent_node.get_office_react_capabilities_with_trace", route)
    monkeypatch.setattr("app.agents.orchestration.react.agent_node.make_skill_tool", fake_tool)
...
def test_office_react_compiles_candidate_boundary_from_registry_contract(monkeypatch):
    ...
    monkeypatch.setattr("app.agents.orchestration.react.runner.get_chat_model", lambda **kwargs: asyncio.sleep(0, result=model))
    monkeypatch.setattr("app.agents.orchestration.react.agent_node.get_office_react_capabilities_with_trace", capabilities)
