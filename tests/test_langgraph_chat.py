...
def _skills():
    ...
    previous_base_only = settings.AGENT_BASE_TOOLS_ONLY
    settings.AGENT_BASE_TOOLS_ONLY = False
    SkillRegistry.clear()
    ToolRegistry.clear()
    ...
    loader.unload_skill_plugins()
    SkillRegistry.clear()
    settings.AGENT_BASE_TOOLS_ONLY = previous_base_only
    ToolRegistry.clear()
...
class _OfficeGraphEchoSkill(Tool):
...
def test_chat_graph_runs_one_tool_at_a_time_and_keeps_tool_data_untrusted(monkeypatch):
    ToolRegistry.register(_GraphEchoSkill())
    model = _ScriptedModel(
        [
            AIMessage(
                content="",
                tool_calls=[
                    {"name": "DateTime", "args": {"text": "first"}, "id": "c1"},
                    {"name": "DateTime", "args": {"text": "second"}, "id": "c2"},
                ],
            ),
    ...
    assert answer == "完成"
    assert citations == []
    assert records == [{"skill": "DateTime", "success": True, "error_code": None, "error": None}]
...
def test_chat_graph_failure_becomes_safe_tool_message_and_max_round_forces_final_answer():
    ...
    ToolRegistry.register(FailingDatetimeSkill())
    model = _ScriptedModel(
        [
            AIMessage(content="", tool_calls=[{"name": "DateTime", "args": {"text": "x"}, "id": "c1"}]),
...
def test_chat_graph_returns_decision_signals_after_a_tool_call():
    ...
    ToolRegistry.register(SignalledSkill())
    model = _ScriptedModel([
        AIMessage(content="", tool_calls=[{"name": "DateTime", "args": {"text": "x"}, "id": "c1"}]),
        AIMessage(content="完成"),
    ])
...
def test_chat_tool_graph_is_intent_gated_and_never_exposes_office_tools():
    ...
    assert names <= {"WebSearch", "WebFetch", "AskUserQuestion", "DateTime", "Calculator"}
